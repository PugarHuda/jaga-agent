...
| `test.mjs` / `test-integration.mjs` / `test-e2e.mjs` | engine (53) / shapes, valuation, filters, config, audit, live paper server incl. LOT_SIZE, replay clock, dashboard auth (85) / Playwright on the real replay stack: approvals, panic, hot reload, metrics, health, webhook alerts, CSRF, MCP tools + resources + prompts, restart reconnect, two-tab double-approve race, headroom panel (41) |
