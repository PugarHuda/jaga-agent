...
| **circuit-breaker** | flash crash: N% drop within a rolling window (N ticks, or `windowSec` real seconds) | liquidate | high |
...
- 🧾 **Tamper-evident audit trail** — every tick, violation, order, decision and report is appended to `audit.jsonl` as a **SHA-256 hash chain**. `npm run audit:verify` pinpoints any edited, deleted or reordered line. Past 50 MB (`--audit-max-mb`) the trail rotates to `audit.jsonl.1` and the chain continues across the split, so long-running deployments stay verifiable end to end.
...
Measured on this machine (`npm run backtest`, 2024-08-04T20:00:00Z +12h, 15-min steps, rogue agent on): market ETH -14.6% / BTC -11.1% / SOL -13.4%; buy-and-hold -6.96%, with Jaga -3.74% (worst tick 6.27% down), 13 sells, rogue pumped $4603 into ETH. **Jaga vs holding: +43.86 USDC (+3.45%).** Numbers vary slightly run to run because the rogue agent and the replay clock are real processes, not a script.
...
| `test.mjs` / `test-integration.mjs` / `test-e2e.mjs` | engine (51) / shapes, valuation, filters, config, audit, live paper server incl. LOT_SIZE, replay clock, dashboard auth (83) / Playwright on the real replay stack: approvals, panic, hot reload, metrics, health, webhook alerts, CSRF, MCP tools + resources + prompts, restart reconnect, two-tab double-approve race, headroom panel (40) |
