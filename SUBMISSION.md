...
Before recording: `set ANTHROPIC_API_KEY=...` (Claude reports show up in the feed), `rm state.json audit.jsonl`, font size up, dashboard at `http://localhost:7777` on one half of the screen, terminal on the other.
...
2. **(0:15–1:00) Live demo.** `npm run demo`. Point at the dashboard: "That red log line is a compromised trading agent piling my portfolio into ETH — 60% concentration. Watch Jaga: detects the breach, executes the trim back to 40% through Binance MCP, logs it to the audit trail, and Claude writes the incident report. Now the market's dropping — stop-loss and circuit breaker fire, everything de-risks to USDC automatically."
...
>
> Code enforces. AI explains. Live dashboard + audit trail + Claude incident reports. MCP-native.
>
